...
public class OStatementCache {
  ...
  /**
   * returns an already parsed server-level SQL executor, taking it from the cache if it exists or creating a new one (parsing and then putting
   * it into the cache) if it doesn't
   *
   * @param statement the SQL statement
   * @param db        the current OrientDB instance. If null, cache is ignored and a new executor is created through statement parsing
   *
   * @return a statement executor from the cache
   */
