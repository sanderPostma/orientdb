...
public class ViewManager {
    ...
    private final Map<String, WatchedClassContext> watchedClassContextMap = new HashMap<>();
    ...
    public synchronized boolean registerLiveUpdateFor(ODatabaseSession db, String viewName) {
        ...
        if (view.getUpdateStrategy() != null
            ...
            try {
                ...
                OLogManager.instance().error(this, "Could not register live query for view %s.", ex, viewName);
            }
            registered = true;
        }
    ...
    public synchronized void updateView(OView view, ODatabaseDocumentInternal db) {
        boolean firstUpdate = !lastUpdateTimestampForView.containsKey(view.getName());
        lastUpdateTimestampForView.put(view.getName(), System.currentTimeMillis());

        String originRidField = view.getOriginRidField();
        if (!firstUpdate && originRidField != null && view.getWatchClasses().size() == 1) {
            try {
                updateViewPartially(view, db);
                return;
            } catch (Exception e) {
                e.printStackTrace();
            }
...
/*
            final StringBuilder statement = new StringBuilder().append(view.getQuery());
            if(!StringUtils.containsIgnoreCase(view.getQuery(), " WHERE ")) {
                statement.append(" WHERE ");
            }
                    .append(" WHERE ").append(view.getOriginRidField()).append(" in [");

 */

    private void updateViewPartially(OView view, ODatabaseDocumentInternal db) {
        final String viewName = view.getName();
        final String originRidField = view.getOriginRidField();
        final String className = view.getWatchClasses().get(0).toLowerCase(Locale.ENGLISH);
        final WatchedClassContext watchedClassContext = watchedClassContextMap.get(className);
        final Map<OIdentifiable, ODocument> savedRecords = watchedClassContext.popSavedRecords();
        if (!savedRecords.isEmpty()) {
            final StringBuilder statement = new StringBuilder().append("SELECT FROM ").append(viewName)
                    .append(" WHERE ").append(view.getOriginRidField()).append(" in [");
            savedRecords.values().forEach(doc -> {
                statement.append(doc.getIdentity()).append(',');
            });
            statement.deleteCharAt(statement.length() - 1);
            statement.append(']');
            try (OResultSet rs =
                         db.query(statement.toString())) {
                while (rs.hasNext()) {
                    OResult row = rs.next();
                    ORID rid = row.getProperty(originRidField);
                    row.getElement()
                            .ifPresent(
                                    elem -> updateViewRow(elem, savedRecords.get(rid), view, db));
                }
            }
        }

        final List<ODocument> deletedRecords = watchedClassContext.popDeletedRecords();
        if (!deletedRecords.isEmpty()) {
            final StringBuilder statement = new StringBuilder().append("DELETE FROM ").append(viewName)
                    .append(" WHERE ").append(view.getOriginRidField()).append(" in [");
            savedRecords.values().forEach(doc -> {
                statement.append(doc.getIdentity()).append(',');
            });
            statement.deleteCharAt(statement.length() - 1);
            statement.append(']');
            Number count = db.command(statement.toString()).next()
                    .getProperty("count");
            // TODO check count
        }
    ...
    private void updateViewRow(OElement viewRow, ODocument doc, OView view, ODatabaseDocumentInternal db) {
        ...
        if (stm instanceof OSelectStatement) {
            ...
            if (projection == null
                ...
                for (String name : doc.getPropertyNames()) {
                    if ("@rid".equalsIgnoreCase(name)
                            || "@class".equalsIgnoreCase(name)
                            || "@version".equalsIgnoreCase(name)) {
                        continue;
                    }
                    Object value = doc.getProperty(name);
                    viewRow.setProperty(name, value);
                }
            } else {
                for (OProjectionItem oProjectionItem : projection.getItems()) {
                    final Object value = oProjectionItem.execute(doc, new OBasicCommandContext());
                    final String name = oProjectionItem.getProjectionAliasAsString();
                    if ("@rid".equalsIgnoreCase(name)
                            || "@class".equalsIgnoreCase(name)
                            || "@version".equalsIgnoreCase(name)) {
                        continue;
    ...
    public void recordAdded(
        ...
        final String className = clazz.getName().toLowerCase(Locale.ENGLISH);
        lastChangePerClass.put(className, System.currentTimeMillis());
        getWatchedClassContext(className).addSavedRecord(doc);
    }
    ...
    private WatchedClassContext getWatchedClassContext(final String className) {
        WatchedClassContext watchedClassContext = this.watchedClassContextMap.get(className);
        if (watchedClassContext == null) {
            watchedClassContext = new WatchedClassContext(className);
            this.watchedClassContextMap.put(className, watchedClassContext);
        }
        return watchedClassContext;
    }


    private class WatchedClassContext {

        private String className;
        private Map<OIdentifiable, ODocument> savedRecords = new HashMap<>();
        private final Lock savedRecordsLock = new ReentrantLock();
        private final List<ODocument> deletedRecords = new ArrayList<>();
        private final Lock deletedRecordsLock = new ReentrantLock();

        public WatchedClassContext(String className) {
            this.className = className;
        }

        public void addSavedRecord(ODocument doc) {
            try {
                savedRecordsLock.lock();
                savedRecords.put(doc.getIdentity(), doc);
            } finally {
                savedRecordsLock.unlock();
            }
        }

        public Map<OIdentifiable, ODocument> popSavedRecords() {
            try {
                savedRecordsLock.lock();
                final Map<OIdentifiable, ODocument> savedRecords = new HashMap<>();
                savedRecords.putAll(this.savedRecords);
        ...
        public void addDeletedRecord(ODocument doc) {
            try {
                deletedRecordsLock.lock();
                deletedRecords.add(doc);
            } finally {
                deletedRecordsLock.unlock();
            }
        }

        public List<ODocument> popDeletedRecords() {
            try {
                deletedRecordsLock.lock();
                final List<ODocument> deletedRecords = new ArrayList<>();
